...
// Two-tier opt-in: user config (~/.pi/agent/extensions/pi-webtools-lite/config.json under
...
export class GitHubInterceptor implements UrlInterceptor {
	...
	private showGhHint(): void {
		if (!this.ghHintShown) {
			...
			console.error("[pi-webtools-lite] Install `gh` CLI for better GitHub repo access including private repos.");
		}
	}
